...
import { RiHome5Line, RiLogoutBoxRLine, RiSettings4Line, RiUser2Line, RiUserLine } from 'react-icons/ri';
        ...
        <div className='mt-4 lg:mt-2 text-sm'>
            {menuItems.map(mitem => (
                <div className='flex flex-col gap-2 my-2' key={mitem.title}>
                    <span className='hidden lg:block text-gray-100 font-medium my-4'>{mitem.title}</span>
                    {mitem.items.map(item => (
                        <Link href={item.path} key={item.label} className='flex items-center justify-center lg:justify-start gap-4 text-white py-2'>
                            {item.icon}
